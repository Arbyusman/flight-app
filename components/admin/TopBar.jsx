  ...
  const whoami = () => {
    ...
    fetch(`${process.env.API_ENDPOINT}api/v1/user`, {
      ...
      .then((data) => {
        ...
        if (data.data.id) {
          fetch(`${process.env.API_ENDPOINT}api/v1/notification/user/${data.data.id}`, {
            method: "GET",
            headers: {
              Authorization: `Bearer ${token}`,
            },
          ...
          <Popover className="relative  justify-center items-center ">
            <Popover.Button
              ...
              {notificationRead.length > 0 ? (
                <div className="inline-flex absolute -top-2  justify-center items-center w-6 h-6 text-xs font-bold text-white bg-red-500 rounded-full border-2 border-white dark:border-gray-900">{notificationRead.length}</div>
              ) : (
                <div></div>
              )}
            </Popover.Button>
              ...
              <Popover.Panel className="absolute  overflow-y-scroll h-80  right-4 z-50 mt-2 -mr-7 bg-white shadow-sm rounded max-w-xs w-screen md:w-screen">
                <div className="relative p-3">
                  <div className="md:flex justify-between shadow-md bg-gray-300 px-4 py-2   sticky top-0 z-10 items-center w-full">
                    <p className="text-gray-700  font-medium text-lg  tracking-normal antialiased items-center justify-center text-center">Notifications</p>
                  </div>
                  <hr></hr>
                    ...
                    <div className="block ">
                      {notification.length > 0 ? (
                        ...
                        notification.map((item) => (
                          <div className="flex items-center justify-start mb-1" key={item.id}>
                            <div
                              className={`flex justify-between gap-2 text-sm shadow-md py-1 px-4 rounded-sm mb-1 items-center text-gray-500 text-left w-full  ${
                                item.isRead === true ? "bg-gray-200 hover:bg-gray-300 text-gray-600 hover:text-gray-700" : "bg-gray-600 hover:bg-gray-700 text-gray-50 hover:text-white"
                              }`}>
                              <span>{item.message}</span>
                              <button
                                className={`bg-gray-200 px-4 py-0.5 rounded-sm hover:bg-gray-300 ${item.isRead === true ? " hidden" : " text-gray-600 hover:text-gray-700"}`}
                                onClick={() => {
                                  {
